...
var otherappfunc = function(){
	...
	//alert("save done!");
	var result = "";
	var url = "/pmkpi/perfprogram/saveIndex.do?tokenid="+getUrlParam('tokenid',responseWind.location.search)+"&authorization="+getUrlParam('authorization',responseWind.location.search);
    if (responseWind.indexdatatable.getEditor()){
        responseWind.indexdatatable.closeEdit();
    }
	var permData = responseWind.indexdatatable.getRecordSet().toArray();
    var editinfo = responseWind.editform.getEditFormValues();
	var commitable=true;
	for(var i=0;i<permData.length;i++){
		var datas = permData[i];
		if(datas.indexval!=undefined && datas.indexval.length > 200){
		    result= "{callsuc:0,callmsg:'指标值不能大于200字！'}";
		    commitable=false;
			break;
		}
	}
    ...
    if (permData && permData.length > 0) {
        for (var a = 0, tleni = permData.length; a < tleni; a++) {
            if(permData[a].levelno == "3"){
                var checkresult = responseWind.Ext.lt.pmkpi.checkSpaceValue(permData[a],"绩效指标-第" + (permData[a]["_sortid"] + 1) + "行", "indexdatatable");
                if (checkresult && checkresult != "success"){//校验空格
                    result = checkresult;
                    return result;
                }
                permData[a].name = convertStr(permData[a].name);
                permData[a].indexval = convertStr(permData[a].indexval);
                permData[a].kpi_val = convertStr(permData[a].kpi_val);
            }
        }
    }
    //处理目标值 特殊字替换
	...
	var perm = JSON.stringify(permData);
	var json = encodeURI(JSON.stringify(permData),"utf-8");
