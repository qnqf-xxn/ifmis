...
var otherappfunc = function(){
    ...
    if(pdmsave != '21') { //21暂存功能
        if(responseWind.totaleditform && !responseWind.totaleditform.check()){
            result= "{callsuc:0,callmsg:'请填写绩效目标！',timeout:1}";
            return result;
        }
        if(responseWind.yeareditform && !responseWind.yeareditform.check()){
            result= "{callsuc:0,callmsg:'请填写绩效目标！',timeout:1}";
            return result;
        }
    }
    ...
    if(responseWind.totaleditform){
        tabcomps["#total"] = Ext.lt.cloneobj(responseWind.totaleditform.getEditFormValues());
    }
    if(responseWind.yeareditform){
        tabcomps["#year"] = Ext.lt.cloneobj(responseWind.yeareditform.getEditFormValues());
    }
    var _yeardata;
    var _totaldata;
    if(pdmsave != '21') { //21暂存功能
        if ((showtab == "1" || showtab == "3")) {
            ...
            var checkresult = responseWind.Ext.lt.pmkpi.checkSpaceValue(_yeardata,"年度绩效目标","yeareditform");//校验空格
        ...
        if ((showtab == "1" || showtab == "2")) {
            ...
            var checkresult = responseWind.Ext.lt.pmkpi.checkSpaceValue(_totaldata,"总体绩效目标","totaleditform");//校验空格
            if (checkresult && checkresult != "success"){//校验空格
                result = checkresult;
                return result;
            }
        ...
        $.ajax({
            ...
            success : function(data) {
                ...
                if (rs && rs.success == true) {
                    ...
                    if(responseWind.totaleditform){
                        responseWind.totaleditform.synchEditform(rtdata);
                    }
                    if(responseWind.yeareditform){
                        responseWind.yeareditform.synchEditform(rtdata);
                    }
