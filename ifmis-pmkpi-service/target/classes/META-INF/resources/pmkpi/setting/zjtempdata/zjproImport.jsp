...
<%
    ...
    String filedownload = "/pmkpi/setting/zjtempdata/file/导入码表项目模板.xlsx";//即将下载的文件的相对路径
    String filedisplay = "导入码表项目模板.xlsx";//下载文件时显示的文件保存名称
    String filenamedisplay = URLEncoder.encode(filedisplay,"UTF-8");
    response.addHeader("Content-Disposition","attachment;filename=" + filedisplay);
*/
    String path = request.getContextPath();
    String menuid = request.getParameter("menuid");
    String is_hubei = request.getParameter("is_hubei");
%>
...
<%--                <a target="_blank" href = "file/导入监审结果-模板.zip" >图片3</a><br>--%>
    ...
    <div class="new-contentarea" style="text-align:center;">
        ...
        <input type="hidden" id="is_hubei" value="<%=path%>">
    </div>
...
<script type="text/javascript">
    ...
    var url = "../exclezjtempcontroller/importExcel.do?tokenid=" + getUrlParam('tokenid', responseWind.location.search);
    url += "&is_hubei="+getUrlParam('is_hubei', responseWind.location.search);
    //点击确定按钮
    ...
    function download() {
        var ishubei = Request("is_hubei"); //是否是湖北
        var str = "";
        var aoa = [];
        if ("1" == ishubei){
            str = "项目支出:01,转移支付：02";
            aoa = [
                ['单位/部门编码', '项目名称', '项目级次', '父级项目','备注',str]
            ];
        } else {
            aoa = [
                ['单位/部门编码', '项目名称', '项目级次', '项目编码', '父级项目编码','备注']
            ];
        }
        var sheet = XLSX.utils.aoa_to_sheet(aoa);
        openDownloadDialog(sheet2blob(sheet), '导入码表项目模板.xlsx');
    }
    function colse() {
        var form = document.forms["demoForm"];
        form.action = "javascript:window.self.close()";
        parent.Ext.lt.pmkpi.query();
        parent.Ext.lt.HTML.unmark();
        //window.opener=null; window.open("",'_self',""); window.close();
        //parent.Ext.lt.ui.close();
