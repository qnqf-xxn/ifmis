...
begin
  ...
  if i = 1 then
	 ...
	 execute immediate 'select fn_altertablecol(''PERF_T_DEPTFUNCTION'', ''name'', ''VARCHAR2'', 300)';
  end if;
  ...
  if i = 1 then
	 ...
	 execute immediate 'select fn_altertablecol(''PERF_T_ADJUSTFUNCTION'', ''name'', ''VARCHAR2'', 300)';
  end if;
