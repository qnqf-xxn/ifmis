...
FROM v_perf_t_review n left join perf_t_audititems m on m.mainguid=n.guid
...
on f.proguid=t.guid
where exists (select * from (select p.pro_code,p.proguid from v_bgt_pm_annual p where p.is_deleted=''2'' group by p.pro_code,p.proguid)n where n.proguid=t.guid)
union all
select
t.guid as guid,--项目guid
t.pro_id as pro_id,--项目id
