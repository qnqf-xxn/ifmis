...
execute immediate 'create or replace view V_PERF_T_MAJORPOLICY as
...
execute immediate 'create or replace view v_perf_project_selfresult as 
select t.*,
...
left join (select a.mainguid as selfevalguid,a.selfresult,a.pro_code from perf_t_selfperfdesc a where a.year = global_multyear_cz.Secu_f_GLOBAL_PARM(''YEAR'')-3)a1 
on t.pro_code = a1.pro_code
left join (select a.mainguid as selfevalguid,a.selfresult,a.pro_code from perf_t_selfperfdesc a where a.year = global_multyear_cz.Secu_f_GLOBAL_PARM(''YEAR'')-2)a2 
on t.pro_code = a2.pro_code
left join (select a.mainguid as selfevalguid,a.selfresult,a.pro_code from perf_t_selfperfdesc a where a.year = global_multyear_cz.Secu_f_GLOBAL_PARM(''YEAR'')-1)a3 
