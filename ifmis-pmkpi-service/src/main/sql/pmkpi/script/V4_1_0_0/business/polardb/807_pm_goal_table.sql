...
begin
  ...
  --select count(1) into i from user_tab_columns where table_name = 'PM_PERF_GOAL_INFO' and column_name = 'BUSI_TYPE_CODE';
  --if i = 1 then
   --  execute immediate 'alter table PM_PERF_GOAL_INFO drop column busi_type_code';
  --end if;
...
--select count(1) into i from user_tab_columns where table_name = 'PM_PERF_GOAL_TEMP' and column_name = 'BUSI_TYPE_CODE';
 -- if i = 1 then
 --    execute immediate 'alter table PM_PERF_GOAL_TEMP drop column busi_type_code';
 -- end if;
