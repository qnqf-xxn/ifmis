num number(9);
begin 
  select count(1) into num from cols where table_name = upper('BGT_PERF_INDICATOR') and column_name = upper('fun_res_code');
  if num = 0 then
     execute immediate 'alter table BGT_PERF_INDICATOR add fun_res_code VARCHAR2(40)';
  end if;
  
  select count(1) into num from cols where table_name = upper('BGT_PERF_INDICATOR') and column_name = upper('indeximport');
  if num = 0 then
     execute immediate 'alter table BGT_PERF_INDICATOR add indeximport VARCHAR2(40)';
  end if;
  
  select count(1) into num from cols where table_name = upper('BGT_PERF_INDICATOR') and column_name = upper('score');
  if num = 0 then
     execute immediate 'alter table BGT_PERF_INDICATOR add score NUMBER(5,2)';
  end if;
  
  select count(1) into num from cols where table_name = upper('BGT_PERF_INDICATOR') and column_name = upper('scorerule');
  if num = 0 then
     execute immediate 'alter table BGT_PERF_INDICATOR add scorerule VARCHAR2(2000)';
  end if;
  
  select count(1) into num from cols where table_name = upper('BGT_PERF_INDICATOR') and column_name = upper('kpi_val1');
  if num = 0 then
     execute immediate 'alter table BGT_PERF_INDICATOR add kpi_val1 VARCHAR2(1000)';
  end if;
  
  select count(1) into num from cols where table_name = upper('BGT_PERF_INDICATOR') and column_name = upper('kpi_val2');
  if num = 0 then
     execute immediate 'alter table BGT_PERF_INDICATOR add kpi_val2 VARCHAR2(1000)';
  end if;
  
  select count(1) into num from cols where table_name = upper('BGT_PERF_INDICATOR') and column_name = upper('kpi_val3');
  if num = 0 then
     execute immediate 'alter table BGT_PERF_INDICATOR add kpi_val3 VARCHAR2(1000)';
  end if;
  
  select count(1) into num from cols where table_name = upper('BGT_PERF_INDICATOR') and column_name = upper('proselect');
  if num = 0 then
     execute immediate 'alter table BGT_PERF_INDICATOR add proselect VARCHAR2(40)';
  end if;
  
  select count(1) into num from cols where table_name = upper('BGT_PERF_INDICATOR') and column_name = upper('isbudget');
  if num = 0 then
     execute immediate 'alter table BGT_PERF_INDICATOR add isbudget VARCHAR2(40)';
  end if;
  
  select count(1) into num from cols where table_name = upper('BGT_PERF_INDICATOR') and column_name = upper('sname');
  if num = 0 then
