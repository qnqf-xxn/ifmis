...
execute immediate 'create or replace view v_perf_t_datacommunication as
select t.*
 from perf_t_datacommunication t
where t.fiscal_year = global_multyear_cz.Secu_f_GLOBAL_PARM(''YEAR'') and t.mof_div_code = global_multyear_cz.Secu_f_GLOBAL_PARM(''DIVID'') and t.is_deleted=2';


execute immediate 'create or replace view v_perf_selfpro_datacommun as
