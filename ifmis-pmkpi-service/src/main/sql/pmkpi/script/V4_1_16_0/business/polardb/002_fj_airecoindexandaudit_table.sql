...
v_sql :='create or replace view PERF_V_AIRECOINDEXMAIN as
SELECT
    t1.*
...
select fn_alterview('PERF_V_AIRECOINDEXMAIN',v_sql) into ln_i;
if ln_i = 0 then
	return;
end if;
...
v_sql :='CREATE OR REPLACE VIEW PERF_V_AIRECOINDEX AS
SELECT
    t1.*
...
and t1.province = global_multyear_cz.v_pmdivid
and t1.year = global_multyear_cz.v_pmyear';
select fn_alterview('PERF_V_AIRECOINDEX',v_sql) into ln_i;
if ln_i = 0 then
	return;
end if;
...
v_sql :='create or replace view V_PERF_T_AISUPPORTAUDITWAITING
as select 
