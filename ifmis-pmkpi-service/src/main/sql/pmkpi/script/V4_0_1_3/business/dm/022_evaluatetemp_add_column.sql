...
num NUMBER;
begin
SELECT COUNT(1) INTO num from user_tab_cols where table_name = upper('PERF_T_EVALUATETEMP') and column_name = upper('ISTOTAL');
 ...
 execute immediate 'create or replace view v_perf_t_evaluatetemp as select * from PERF_T_EVALUATETEMP t where year = global_multyear_cz.Secu_f_GLOBAL_PARM(''YEAR'')
  ...
  END IF;
 SELECT COUNT(1) INTO num from user_tab_cols where table_name = upper('PERF_TMP_EVALUATETEMP') and column_name = upper('ISTOTAL');
