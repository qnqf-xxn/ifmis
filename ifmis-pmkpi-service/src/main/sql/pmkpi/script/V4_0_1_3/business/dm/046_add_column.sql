...
num NUMBER;
 ...
 --PERF_T_PROINDEXTRACE
    SELECT COUNT(1) INTO num from user_tab_cols  where table_name = upper('PERF_T_PROINDEXTRACE') and column_name = upper('COMPLETEANALYSCON');
 ...
 execute immediate 'create or replace view v_perf_t_proindextrace as select * from PERF_T_PROINDEXTRACE t where year = global_multyear_cz.Secu_f_GLOBAL_PARM(''YEAR'')
  ...
  END IF;
  --PERF_T_PROINDEXTRACE
    SELECT COUNT(1) INTO num from user_tab_cols  where table_name = upper('PERF_T_PROINDEXTRACE') and column_name = upper('DEVIATIONANALYSIS');
  ...
  END IF; 
  --PERF_T_PROBGTFUND
    SELECT COUNT(1) INTO num from user_tab_cols  where table_name = upper('PERF_T_PROBGTFUND') and column_name = upper('ECONOMIC');
