...
(select q.name from perf_enum q where q.guid=m.freview and q.elementcode=''REVIEWOPINION'') as freview,
(select q.name from perf_enum q where q.guid=m.sreview and q.elementcode=''REVIEWOPINION'') as sreview 
...
(select q.name from perf_enum q where q.guid=m.audittype and q.elementcode=''AUDITTYPE'') as name 
from v_perf_t_review n left join perf_t_evaluateaudit m on m.proguid=n.proguid 
WHERE  n.type=''assessment'') f
on f.proguid=t.guid
union all
select
t.guid as guid,--项目guid
