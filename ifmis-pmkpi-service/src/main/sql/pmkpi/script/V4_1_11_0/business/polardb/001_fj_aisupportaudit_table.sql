...
select fn_alterview('V_PM_PERF_INDICATORAGG',v_sql) into ln_i;
if ln_i = 0 then
	return;
end if;
...
select pro_code,province,year, xmlagg(xmlparse(content ''指标编码：''||t.code||'',指标名称：''||t.name||'',运算符号：''||(select a.name
    ...
    order by 1).getclobval() as perfindex
FROM perf_t_selfperfindex t where t.province = global_multyear_cz.v_pmdivid and year = (global_multyear_cz.v_pmyear-1)
