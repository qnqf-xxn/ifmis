...
i number;
begin
select count(*) into i from user_views  t where t.VIEW_NAME='V_PM_PROJECT_INFO_FORBDM';
if i=1 then
execute immediate '
...
select t.guid as guid, --项目guid
  ...
  left join (SELECT m.proguid,
    ...
    on f.proguid = t.guid
    left join v_bgt_tra tra on tra.PRO_CODE=t.PRO_CODE
 where exists (select p.pro_code, p.proguid
                  from v_bgt_pm_annual p
                 where p.is_deleted = ''2''
         and p.pro_code = t.pro_code)';
...
WHERE  n.type=''dept'' and n.islast=1 and n.isconfirm=''1'') b on b.proguid = t.guid';
