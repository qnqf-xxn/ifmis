...
<script type="text/javascript"
		src="js/xlsx.core.min.js"></script>
<script type="text/javascript"
...
<body style="height:450px;">
    <form id="demoForm" name="demoForm" httpMethod="post"  enctype="multipart/form-data" action="javascript: uploadAndSubmit();" style="height:350px;">
        <div class="page-wrap">
			<div class="step-wrap">
				...
				<div class="cont first-step-cont">
					<p>如果没有文件模板文件，请下载
						<a href="javascript:void(0);" onclick="javascript:downloadxls();">excel</a>模板文件
				...
				<div id="filepathsta" class="progressbar cont third-setp-cont">
					<div id="div1" class="div1" style="float:left;width:480px">
					</div>
					<div id="div2" class="div2" style="float:left;width:80px">
					</div>
				</div> 
				</div>
		</div>
        ...
        <div class="new-contentarea" style="text-align:center;">
            ...
            <input type="file" name="file" id="upload-file" accept=".xls,.xlsx" multiple style="right:50%;margin-right:-83px;" />
        ...
        <div id="executes" class="execute">
			<input type="button" value="导入完成"  onclick="startScroll(this);"/>
        </div>
    </form>         
    ...
    var url = "../fileController/fileUpload.do?tokenid="+getUrlParam('tokenid',responseWind.location.search);
	...
	function downPager() {
		...
		xhr.open('GET', window.location.search||"//src/main/src/gov/mof/fasp2/pmkpi/randomComment/controller/项目编码-项目名称-上传日期.xlsx", true);
		...
		xhr.onreadystatechange = function () {
			if (xhr.readyState == 4) {
				if (xhr.status == 200) {
					...
					eleLink.download = "项目编码-项目名称-上传日期.xlsx";
	...
	function downloadxls() {
		...
		/*var aoa = [
			['一级指标', '二级指标', '三级指标', '指标性质', '指标值','计量单位','指标权重','抽评核实完成值','抽评得分','备注'],
			['绩效指标','产出指标'],
			['绩效指标','效益指标'],
			['绩效指标','满意度指标']
		];*/
		var sheet = XLSX.utils.aoa_to_sheet(aoa);
		openDownloadDialog(sheet2blob(sheet), '（自行修改）项目编码&项目名称&上传日期.xlsx');
	}
		...
		if (form.files.length > 0) {
			...
			for ( var i = 0; i < form.files.length; i++) {
				...
				if(fileSplit.length == 2){
					...
					var fileNamespl = fileSplit[0].split("&");
					if(fileNamespl.length == 3){
						//判断项目编码
		     			if(/.*[\u4e00-\u9fa5]+.*$/.test(fileNamespl[0])) {
		     				$("#message").html("导入失败...");
		     				arr.push(fileName+"&&项目编码不能含有汉字&&1")
		     				flag++;
			     			//SetMessage("项目编码不能含有汉字...");
							//return;
		   				}
		   				//项目名称无限制
		   				//绩效类型
		   				//if(!(fileName[2] == "项目支出" || fileName[2] == "政策绩效" || fileName[2] == "部门整体")){
		   				//	SetMessage("项目绩效类型只能为'项目支出'或'政策绩效'或'部门整体'...");
						//	return;
		   				//}
		   				//判断是否为日期
		     			if( /^\d+$/.test(fileNamespl[2]) && fileNamespl[2].length == 8) {
		     				/*if(!Date.parse(fileName[2])){
			     				SetMessage("文件日期格式错误...");
								return;
		     				}*/
		   				}else {
		   					$("#message").html("导入失败...");
		   					arr.push(fileName+"&&文件日期必须为8位有效时间数字(20210101)&&1")
		     				flag++;
		   					//SetMessage("文件日期必须为8位有效时间数字(20210101)...");
							//return;
		   				}
					}else {
						$("#message").html("导入失败...");
						arr.push(fileName+"&&文件名称格式错误(项目编码&项目名称&文件日期)&&1")
		     			flag++;
						//SetMessage("文件名称格式错误(项目编码-项目名称-绩效类型-文件日期)...");
						//return; 
					}
				} else {
					$("#message").html("导入失败...");
	...
	function startScroll(btn){
		debugger;
		var leg = window.parent.document.getElementsByClassName('windpblue').length;
		var el = window.parent.document.getElementsByClassName('windpblue');
		el[leg-1].style.display='none';
