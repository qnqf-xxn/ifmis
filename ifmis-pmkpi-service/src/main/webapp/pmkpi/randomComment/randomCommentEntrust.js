...
Ext.lt.pmkpi.randomComment.entrustIndex = function(config, service){
    ...
    var _workflow = config.workflow,
        ...
        _agency,
        _prokind,
        _tablecode = 'V_PERF_T_RANDOMCOMMENT_PROJ',
        _tabcode,
        _tabfilter,
        _tabcode,
		_name,
    ...
    tabChange = function (obj) {
        ...
        if (_lefttabtype == "program") {
            data = _proqfdatas;
            key = '/pmkpi/randomcomment/entrust/index';
        } else if (_lefttabtype == "dept") {
            key = '/pmkpi/randomcomment/entrust/dept';
            data = _deptqfdatas;
        }
        queryfrom(data, key);
...
//        params.prokind = _prokind;
        ...
        params.beanid = "pmkpi.randomcomment.RandomCommentEntrustBOTX";
    ...
    cmp.entrust = function (obj){
		...
		if(size>0){
	        ...
	        Ext.lt.ui.openModalWindow(url,scrWidth,scrHeight,"委托第三方")
		}else{
	...
	cmp.back = function (){
		...
		if(size>0){
			...
			Ext.lt.ui.confirm('是否删除已选择的抽评数据？',null,function(is){
				if(is){
					var config = {};
					config.datas = data;
