...
Ext.lt.pmkpi.randomComment.scopeIndex = function(config, service){
    ...
    tabChange = function (obj) {
        ...
        if (_lefttabtype == "program") {
            data = _proqfdatas;
            key = '/pmkpi/randomcomment/scope/index';
            //_uikey = "/pmkpi/perfself/apply/edit";
        } else if (_lefttabtype == "dept") {
            key = '/pmkpi/randomcomment/scope/dept';
...
//    	_selectedtree = activedics;
        ...
        params.beanid = "pmkpi.randomcomment.RandomCommentScopeBOTX";
...
//        var config = obj.config;
		...
		if (data.length <= 0) {
			Ext.lt.ui.alert('请选择要标识的数据！');
			return;
		} 
		Ext.lt.ui.confirm('是否标识已选择的绩效目标数据？',null,function(is){
			if(is){
				var config = {};
				/*var list = [];
				for(var i=0;i<data.size();i++){
					var param = {};
					param.perfgoalguid = data[i].perfgoalguid;
					list.push(param);
				}*/
				config.list = data;
				config.isscope = "1";
				config.tablecode = _tabcode;
				config = Ext.lt.RCP.asyncall(serviceid , "submitScope", config);
				if(config.error){
