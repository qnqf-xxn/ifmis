...
if (Ext.lt.pmkpi.common == null) {
	Ext.lt.pmkpi.common = {}
};

Ext.lt.pmkpi.common.uploadfiles = function(config,service){
	var cmp = {cfg:config};
	...
	var fudsupload = {
		...
		button_width : "78",
		button_height : "28",
		button_placeholder_id : "新增附件1",
		button_text : '<span class="theFont" ></span>',
		button_text_style : ".theFont { font-size: 15; }",
		button_text_left_padding : 12,
		button_text_top_padding : 3,
		upload_button_image : _APPID_PATH_+"/busfw/ltext/swfupload/images/upload.png",
		_buttons : [],
...
Ext.lt.message.hook("datatable",'beforesetcols',function(resp){
	if(resp.table.key=="/busfw/common/upload/index" || resp.table.key == "/busfw/common/upload/zpindex"){
		var cols=resp.cols;
		for(var i=0; i<cols.length; i++){
			var col = cols[i];
			if(col["colcode"]=="filename"){
				col.fn = function(l,c,data,col){
					return "<a onclick='downloadfile(\""+ data.guid + "\")' href='#'>"+col+"</a>";
				}
			}
		}
	}
