...
Ext.lt.pmkpi.financialevaluation.resultservice = function(config, service){
    var cmp = {};
    ...
    var _menuid = config.menuid,//菜单id
        ...
        _isreport = config.isreport, //是否财政终稿报告上传菜单：1-是；其它-否；
        _resulthide = config.resulthide, // result-财政评价成果审核 隐藏按钮，
        _reporthide = config.reporthide, // report-财政终稿报告上传 隐藏按钮；
    ...
    tabChange = function (obj) {
        ...
        common.changTooltoolbuttontStatus(0,  _lefthidebtn + "," + _tabhidtbtn + _hidebutton);
    ...
    $(function (){
        if(_isreport == 1){ //是否财政终稿报告上传菜单：1-是；0-否；
            _hidebutton = _reporthide;  //财政终稿报告上传
        }else {
            _hidebutton = _resulthide;  //财政评价成果审核
        }
    ...
    Ext.lt.pmkpi.query = function(){
        ...
        params.beanid = "pmkpi.evaluation.financial.ResultBOTX";
    ...
    cmp.clickTabpage = function(obj){
        ...
        _tabcode = obj.code;
        if (!isfirst) { //首次不走查询
            common.changTooltoolbuttontStatus(0,_tabhidtbtn + _hidebutton);
            Ext.lt.pmkpi.query();
        }
    }
    ...
    cmp.wfAudit = function (obj){
        ...
        if (actionname == "审核") {
            ...
            url = Ext.lt.pmkpi.genPageUrl(url, params);
            Ext.lt.ui.openModalWindow(url,scrWidth*0.7,scrHeight*0.7,"财政评价结果审核-"+_lefttreename);
        } else {
            if (datas.length <= 0) {
    ...
    cmp.feedback = function (obj){
        ...
        params.prolev = _prolev;
        params.bustype = _lefttabtype;
        params.audittype = "feedback";
        var url = "/pmkpi/evaluation/financial/auditedit.page";
        url = Ext.lt.pmkpi.genPageUrl(url, params);
        Ext.lt.ui.openModalWindow(url,scrWidth*0.7,scrHeight*0.7,"反馈意见-"+_lefttreename);
    }
    ...
    cmp.reportupload = function(obj){
        ...
        var url = "";
        if (_lefttabtype && "program" == _lefttabtype) { //项目支出
            url = "/pmkpi/evaluation/financial/report/edit.page";
        } else if (_lefttabtype && "dept" == _lefttabtype) { //部门整体
            url = "/pmkpi/evaluation/financial/report/deptedit.page";
        }
        url = Ext.lt.pmkpi.genPageUrl(url, params);
        Ext.lt.ui.openModalWindow(url, scrWidth, scrHeight, "上传结果-" + _lefttreename);
    }
    ...
    cmp.acceptnotice = function (obj) {
        ...
        params.datas = datas;
        params.type = type;
        Ext.lt.RCP.call(service , "acceptNotice", params, function(rs) {
            if (rs && rs.success) {
