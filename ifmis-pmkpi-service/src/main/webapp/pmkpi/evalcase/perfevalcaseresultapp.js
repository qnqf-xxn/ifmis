...
if(!Ext.lt.pmkpi.evalcaseresultappservice){
    Ext.lt.pmkpi.evalcaseresultappservice = {};
}

Ext.lt.pmkpi.evalcaseresultappservice = function(config, service){
    ...
    $(function (){
        ...
        $('.tbtitle').append("<p title='' style=\"color: red;float:right;\" class=\"layui-text\">以下内容至少填写一项</p>");
        if (_viewtype == "query") {
            try{
    ...
    Ext.lt.pmkpi.query = function(isquery){
        ...
        Ext.lt.RCP.call(service , "getFundDatas", params, function(rs){
    ...
    cmp.save = function(){
       if (!cmp.checkAllDatas()){return ;} //先校验
        ...
        params.procode = _procode;
        Ext.lt.ui.loadingShow();
        var rs = Ext.lt.RCP.asyncall(service , "saveFundDatas", params);
        Ext.lt.ui.loadingClose();
        if (rs && rs.success) {
            Ext.lt.pmkpi.query();
        }
