...
var delyear = []; //删除的年度目标  deptperf
var _weight_sum;

Ext.lt.pmkpi.selfindexdept = function (config, service) {
    ...
    cmp.checkall = function () {
        ...
        var ccweight = 0;
        var params = {};
        params.frametype = "DEPT";
        params.name = "产出指标";
        var cccode = Ext.lt.RCP.asyncall(serviceid, "getFrameCode", params);
        for (var k = 0, lenk = yeargoals.length; k < lenk; k++) {
            ...
            for (var i = 0, leni = datas.length; i < leni; i++) {
                /**
                 * 2、保存时，校验所有套绩效目标的产出指标权重之和等于40分，当不符合校验规则时提示“所有年度绩效目标产出指标权重之和应等于40分，请检查。”。（可直接写死不需要动态配置）
                 * 3、保存时，校验所有套绩效目标的指标权重之和等于80分，当不符合校验规则时提示“所有年度绩效目标指标权重之和需等于80分，请检查。”。（可直接写死不需要动态配置）
                 */
                if(datas[i]["levelno"] == 3){
                    // if(datas[i]["score"] == 0){
                    //     alert("得分不能为零！");
                    //     alert("第" + (k + 1) + "行目标的第" + (datas[i]["_sortid"] + 1) + "行的绩效指标得分不能为零");
                    //     return false;
                    // }
                    allweight += datas[i].actualweight ? getPointNum(parseFloat(datas[i].actualweight)) : 0.00;
                    if(datas[i]["findex"] == cccode){ // 新旧指标层次的产出指标code
                        ccweight += datas[i].actualweight ? getPointNum(parseFloat(datas[i].actualweight)) : 0.00;
                    }
                }
        ...
        if (ccweight != 40) {
            alert("所有年度绩效目标产出指标权重之和应等于40分，请检查。");
            return false;
        }
        if (allweight != 80) {
            alert("所有年度绩效目标指标权重之和需等于80分，请检查。");
            return false;
        }
        return true;
    }
        ...
        for (var i = 0; i < cols.length; i++) {
            ...
            if (col["colcode"] == "actualvalue") { // 实际完成值
                ...
                col.oneditend = function (table, el, l, c, d) {
                    ...
                    if (d.computesign == "6" && d.actualweight && d.score) { //校验得分区间
                        return qualCheck(d);
                    }
                }
            }
            if (col["colcode"] == "actualweight") {
                ...
                col.oneditend = function (table, el, l, c, d) {
                    ...
                    if (d.computesign == "6" && d.actualvalue && d.score) { //校验得分区间
                        return qualCheck(d);
                    }
                }
            }
...
function qualCheck(d) {
    ...
    if (warndata && warndata != "1") {
         ...
         * 达成年度指标                XX指标：MM指标的实际得分区间为[权重*80%(含)-权重]，请调整后再保存。
         ...
         --湖北
         达成预期指标	手工录入，校验应为权重分的100-80%（含80%）
         部分达成预期指标并具有一定效果	手工录入，校验应为权重分的80-50%（含50%）
         未完成预期指标且效果较差	手工录入，校验应为权重分的50-0%
