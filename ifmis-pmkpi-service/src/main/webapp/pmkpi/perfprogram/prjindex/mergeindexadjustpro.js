...
Ext.lt.pmkpi.treeindexadjustpro = function(config, serviceid) {
    ...
    var _adjmainguid;
    var _adjusttype = config.adjusttype;
    ...
    $(function() {
        ...
        common.initDatatable(befortable);
    ...
    _cmp.init = function(){
        ...
        params.adjusttype = _adjusttype;
        params.viewtype = _viewtype;
        //Ext.lt.ui.loadingShow();
        Ext.lt.RCP.call(serviceid, "findProAdjustIndex", [params], function(data) {
            tabcomps = data;
            tabcomps["del#total"] = []; //总体删除
            tabcomps["del#year"] = []; //年度删除
    ...
    savealltab = function(){
        ...
        if(tempgoal["kpi_target"] == "" || !tempgoal["kpi_target"]){
            alert("项目变更缺少项目总体绩效数据不能保存，请检查项目库是否填报总体绩效！");
            return;
        }
        //总体绩效指标名称重复校验
        var repetMsg =_cmp.getRepetitionMsg(tabcomps["#total"]);
        if(repetMsg){
            alert("总体绩效 '"+repetMsg+"' 指标名称重复，无法保存！");
            return;
        }
        var params = {};
    ...
    addperformance = function(){
       ...
       var url = httpurl+ Ext.lt.token.urlAddToken('/pmkpi/program/report/adjustindex.page?state='+_state+
           "&ProCat="+procat+"&setyear="+_setyear+"&admdiv="+_admdiv+"&ISXZZD="+_ISXZZD+"&itemid="+_proguid+"&usercode="+_usercode
           +"&showtab="+showtab+"&isbefore="+_isbefore+"&isedittotal="+_isedittotal+"&noreadonly=1&adjusttype=1")+ Ext.lt.token.getAuthorization();
                ...
                if(col["colcode"]=="option_value"){
                    col.fn = function(l,c,data,col){
                        ...
                        if(data.datatable_do_sum==true){
                            ...
                            return "<span style=\"margin-left:10px;\">是</span>";
                        }else if(data.option_value == '0'){
                            return "<span style=\"margin-left:10px;\">否</span>";
                        }else{
                            return "";
                        }
                    }
                }
                if(col["colcode"]=="adjust_opt_value"){
                    col.fn = function(l,c,data,col){
                        var yrs = '&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<label><input type="radio" id="adjoption_value_y'+data.ordernum+'" name="adjoption_value_radio'+data.ordernum+'" value="1">是</label>';
                        var yrsc = '&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<label><input type="radio" id="adjoption_value_y'+data.ordernum+'" checked="checked" name="adjoption_value_radio'+data.ordernum+'" value="1">是</label>';
                        var nrs = '&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<label><input type="radio" id="adjoption_value_n'+data.ordernum+'" name="adjoption_value_radio'+data.ordernum+'" value="0">否</label>';
                        var nrsc = '&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<label><input type="radio" id="adjoption_value_n'+data.ordernum+'" checked="checked" name="adjoption_value_radio'+data.ordernum+'" value="0">否</label>';
                        if (_viewtype == "query") {
                            if(data.datatable_do_sum==true){
                                return "";
                            }else if(data.adjust_opt_value == '1'){
                                return "<span style=\"margin-left:10px;\">是</span>";
                            }else if(data.adjust_opt_value == '0'){
                                return "<span style=\"margin-left:10px;\">否</span>";
                            }else{
                                return "";
                            }
                    ...
                    col.onclick = function (table, el, l, c, d) {
                        ...
                        var option_value = $("input[name='adjoption_value_radio"+d.ordernum+"']:checked").val();
                        if(option_value == '1'){
                            d.adjust_opt_value = '1';
                            d.adjust_score = d.weight;
                        }else{
                            d.adjust_opt_value = '0';
                            d.adjust_score = '0';
                        }
    ...
    exportIndexFile = function () {
        ...
        var _templatecode;
        if(_curtab == "total"){
            _templatecode = "hbproadjtotalgoalandindex";
        }
        var input2=$("<input>");
...
function beforeamtTotal(table) {
    if(table.getSumobj()){
        ...
        for ( var i = 0; i < len; i++) {
            ...
            var adjust_score = rowarr[i]["adjust_score"] ? rowarr[i]["adjust_score"] : 0;
            sumadjscore += rowarr[i]["adjust_score"] ?  parseFloat(adjust_score) : parseFloat(unit_since_score);
            var lev_score = rowarr[i]["adjust_score"] ?  adjust_score : unit_since_score;
            sumlevscore += parseFloat(lev_score);
        }
        var totalrs = {};
        totalrs["weight"] = sumweight == 0 ? "" : getPointNum(sumweight);
        totalrs["unit_since_score"] = sumscore == 0 ? 0 : getPointNum(sumscore);
        totalrs["adjust_score"] = sumadjscore == 0 ? 0 : getPointNum(sumadjscore);
        ...
        //处理等级
        var levtitle = "";
        var levsorce = getPointNum(sumlevscore);
        if (levsorce >= 90){
            levtitle = "等级：优";
