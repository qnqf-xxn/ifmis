...
Ext.lt.pmkpi.adjust.auditservice = function(config, service){
    ...
    Ext.lt.pmkpi.query = function(){
        ...
        params.tabfilter1 = _tabfilter;
        params.workflow = _workflow;
        params.lefttabtype = _lefttabtype;
        params.leftmenuid =_leftmenuid;
        params.beanid = "pmkpi.adjust.audit.AdjustAuditBOTX";
    ...
    cmp.setTableColum = function(){
        ...
        for (var i = 0, len = cols.length; i < len; i++) {
            ...
            if (_isNINGXIA && _lefttabtype == "program") {
                if (col.colcode == "delopinion" || col.colcode == "delwfstatus" || col.colcode == "delauditopinion") {
                    _displaycols.push(col); //隐藏前暂存至sfagency;
                    col.display = false; //display置为false，隐藏
                }
            }
    ...
    cmp.wfAudit = function (obj){
        ...
        if (actioncode == "cancelaudit" && "011" == wfstatus){
    ...
    cmp.audit = function (obj){
        ...
        params.projectkind = data[0].projectkind;
        if (_lefttabtype && "program" == _lefttabtype) { //项目支出
            params.uikey = "/pmkpi/adjust/report/proedit";
        } else if (_lefttabtype && "dept" == _lefttabtype) { //部门整体
            params.uikey = "/pmkpi/adjust/report/deptedit";
        }
