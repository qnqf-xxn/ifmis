...
Ext.lt.pmkpi.adjusttraindex = function (config, serviceid) {
    var _cmp = {};
    ...
    var _mainguid = config["mainguid"];//项目编码
            ...
            if (iframe && iframe.childNodes && iframe.childNodes[0] && iframe.childNodes[0].contentDocument) {
                ...
                if (url) {
                    ...
                    if (iframe.contentWindow) {
                        if (url == "/pmkpi/adjust/report/proedit/index") {
                            indexdatas = iframe.contentWindow.datatable.getRecordSet().toArray();
                        } else if (url == "/pmkpi/adjust/report/proedit/goal") {
                            goaldata = iframe.contentWindow.editform.getEditFormValues();
                            if (goaldata && (!goaldata.adjustndgoal || goaldata.adjustndgoal == "")) {
                                goaldata = undefined;
                            }
                        }
                    }
    ...
    _cmp.save = function (obj) {
        ...
        if (_treeid) {
            ...
            Ext.lt.ui.loadingClose();
            data.goal = goal;
            data.indexs = datatable.getRecordSet().toArray();
            datasmap[_treeid] = data;
            return rs;
        } else {
            return {success : true};
    ...
    _cmp.check = function () {
        ...
        if (datas.length == 0) {
            Ext.lt.ui.alert("区划列表数据未加载，请刷新页面后重试！", {timeout: 1}, function () {
            });
            return false;
        }
    ...
    _cmp.impdata = function (obj) {
        ...
        if (!checksub) {
            alert("总体年度绩效指标发生了调整，请先点击'获取最新调整整体绩效指标'后，再进行导入！");
            return;
        }
        var config = eval('(' + obj.config + ')');
        var params = {};
        params.beanid = "pmkpi.adjust.AdjustTraIndexBOTX";
    ...
    _cmp.expdata = function(obj){
        ...
        if (_viewtype != "query" && ((datasmap.submofs && datasmap.submofs.length > 0) && (!_indexdatas || _indexdatas.size == 0 || !_goaldata))) {
            alert("请填写年度绩效目标、指标后再导出“调整细化区域年度绩效”！");
            return;
        }
        var checksub = true;
        for (var i=0; i<_submofdatas.length; i++) {
