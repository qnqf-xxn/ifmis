...
Ext.lt.pmkpi.workevaluate.audit = function(config, service){
    ...
    Ext.lt.pmkpi.query = function(){
        ...
        params.beanid = "pmkpi.workevaluate.audit.WorkEvaluateAuditBOTX";
    ...
    //送审/审核
    Ext.lt.pmkpi.wfAudit = function (obj){
        var btnname = obj.name;
        _actioncode = obj.code;
        var datas = [];
        ...
        if (datas.length <= 0) {
            Ext.lt.ui.alert("请选择要" + btnname + "的数据！",function(){});
            return;
        }
    ...
    //审核
    cmp.auditOrDetailed = function (obj){
        var btnconfig = eval('(' + obj.config + ')');
        var url = "";
        var params = {};
        var name = "请选中一条数据进行审核！";
        if (obj.code == "detailed") {
            ...
            if (_lefttabtype && "dept" == _lefttabtype) { //预算部门
                url = "/pmkpi/workevaluate/audit/deptedit.page";
            } else if (_lefttabtype && "subprovince" == _lefttabtype) { //下级财政
                params.provincescoretype = _provincescoretype;
                url = "/pmkpi/workevaluate/audit/subprovinceedit.page";
            }
        }
