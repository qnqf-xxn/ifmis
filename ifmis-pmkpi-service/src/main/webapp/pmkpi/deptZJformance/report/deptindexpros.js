...
Ext.lt.pmkpi.deptselpros = function(config, service) {
    ...
    $(document).ready(function(){
        ...
        if(_SYSTEMSTYLE_ == "grey"){
            $("#queryform_div").find(".searchLeft").css("width","50%");
        }
    });
    //右侧区域填写html脚本
    Ext.lt.pmkpi.initMainDiv = function() {
        ...
        if(_SYSTEMSTYLE_ == "grey"){
            headhtml += ' <tr><th width="45%" align="left"><div style="width: 13%;float:left;background-color:#fff;border:none;" class="busquerypanel_outline"><span>&nbsp;待选项目：</span></div>';
            headhtml += ' <div id="queryform_div" style="width: 86%;float:right;"></div>';
            headhtml += ' </th><th width="10%"></th><th width="45%" align="left"><div style="width: 13%;float:left;background-color:#fff;border:none;" class="busquerypanel_outline"><span>&nbsp;已选项目：</span></div></th></tr> ';
        }else{
            headhtml += ' <tr><th width="45%" align="left"><span>&nbsp;待选项目：</span>';
            headhtml += ' <div id="queryform_div" style="margin-top:-12px;"></div>';
            headhtml += ' </th><th width="10%"></th><th width="45%" align="left">&nbsp;已选项目：</th></tr> ';
        }
        headhtml += ' <tr><td colspan="3" height="6px" style="height: 20px;"></td></tr>';
     ...
     * 根据查询条件初始化未选择项目
     */
    cmp.initnoseltableData = function () {
        var params = {};
        ...
        Ext.lt.RCP.call(serviceid, "getDatas", params, function(data) {
            ...
            for (var i = 0; i < projectsdata.length; i++) {
                tempproid = projectsdata[i]["pro_code"];
                if (!selguids.indexOf(tempproid) > -1) {
                    noseldatas.push(projectsdata[i]);
                }
            }
            noseltable.setDatas(noseldatas);
            Ext.lt.ui.loadingClose();
        });
    }

    Ext.lt.message.hook("queryform", "doquery",function(obj){
        cmp.initnoseltableData();
    });
