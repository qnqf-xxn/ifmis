...
Ext.lt.pmkpi.deptZJperfedit = function(config, service){
    ...
    indimport = config["indimport"],//指标重要性
	...
	cmp.clickTabpage = function(obj){
        ...
        if (_viewtype == "query") {
            $("#tabtoolbutton").hide();
        }else if(_onclickid != 'goaleditform' && _onclickid != 'perfindexdatatable'){
            $("#tabtoolbutton").hide();
        }else {
            $("#tabtoolbutton").show();
        }
               ...
               /* if(col["colcode"] == "sname"){ //二级
                    ...
                    col.cseditfn = function(i,j,rs, style){ //列的单元格样式.
                        if(j==3){
                            ...
                            var statnum = 0;
                            if(i==statnum){
                                sindex = Ext.lt.cloneobj(rs["sname"]+rs["fun_res_code"]);
                            }
                            if(sindex == (rs["sname"]+rs["fun_res_code"])){
                                style ='style="border-bottom-style:none;text-align:center;';//position:relative;padding-top:5px;
                            } else {
                                style ='style="border-top-width:1px;text-align:center;border-top-color:#cccccc;border-bottom-style:none;';
                            }
                    ...
                    col.fn = function(i,c,rs,value){ //行的显示样式.
                        if(rs["sname"]){
                            subsindex = Ext.lt.cloneobj(rs["sname"]+rs["fun_res_code"]);
                            var seq = showMidleName2(subsindex);
                            if(i==seq){
                                return value;
                            }
                            return "";
                ...
                /*if(col["colcode"] == "name"){
                    ...
                    col.cseditfn = function(i,j,rs, style){ //列的单元格样式.
                        if(j==4){
                            ...
                            var statnum = 0;
                            if(i==statnum){
                                sindex = Ext.lt.cloneobj(rs["name"]);
                            }
                            if(sindex == rs["name"]){
                                style ='style="border-bottom-style:none;text-align:center;';//position:relative;padding-top:5px;
                            } else {
                    ...
                    col.fn = function(i,c,rs,value){ //行的显示样式.
                        if(rs["name"]){
                            subsindex = Ext.lt.cloneobj(rs["name"]);
                            var seq = showMidleName(subsindex,"name");
                            if(i==seq){
                                return value;
                            }
                            return "";
    ...
    cmp.checkAll = function() {
        ...
        for (var i = 0,leni = datas.length; i < leni; i++) {
            ...
            if (!!rowdata["kpi_val"] && rowdata["kpi_val"].match(RegExp("[*_-]")) != null) {
     ...
     * 复制上年绩效
     */
    cmp.copyperf = function(obj){
        var params = {};
        params.mainguid = _mainguid;
        params.saveAgency = _saveAgency; //部门编码
        if (_onclickid == 'goaleditform'){ //复制目标
            params.copytype = 'goal';
        }else if (_onclickid == 'perfindexdatatable'){ //复制指标
            params.copytype = 'index';
        }
        Ext.lt.ui.confirm("是否复制上年绩效目标到年度绩效指标？",{icon:"warn"},function(ret){
            if(ret){
                var rs = Ext.lt.RCP.asyncall(serviceid, "copyPerf", params);
                if(rs.success){
                    if (_onclickid == 'goaleditform'){ //复制目标
                        if(rs.data.length>0){
                            goaleditform.synchEditformByObj(rs.data[0]);
                            Ext.lt.ui.alert("复制成功！",function(){});
                        }else{
                            Ext.lt.ui.alert("上年绩效目标为空！",function(){});
                        }
                    }else if (_onclickid == 'perfindexdatatable'){ //复制指标
                        if(rs.data.length>0){
                            var selDatas = perfindexdatatable.getRecordset().toArray();
                            perfindexdatatable.removeData(selDatas);
                            perfindexdatatable.addData(rs.data);
                            var ind_source = [];
                            ...
                            var indimport_source = [];
                            for(var i=0; i<indimport.length; i++){
                                var temparr = [];
                                temparr.push(indimport[i]["guid"]);
                                temparr.push(indimport[i]["code"]);
                                temparr.push(indimport[i]["name"]);
                                indimport_source.push(temparr);
                            }
                            perfindexdatatable.addMapperDatas("indeximport",indimport_source);
                            var funcresp_source = [];
                            for(var i=0; i<funcresp.length; i++){
                                var temparr = [];
                                temparr.push(funcresp[i]["guid"]);
                                temparr.push(funcresp[i]["code"]);
                                temparr.push(funcresp[i]["name"]);
                                funcresp_source.push(temparr);
                            }
                            perfindexdatatable.addMapperDatas("fun_res_code",funcresp_source);
                            var zbxs_source = [];
                            ...
                            Ext.lt.ui.alert("上年绩效指标为空！",function(){});
                        }
                    }
                } else {
