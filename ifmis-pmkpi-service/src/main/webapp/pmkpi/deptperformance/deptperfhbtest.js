...
var delyear = []; //删除的年度目标
Ext.lt.pmkpi.deptperftest = function (config, service) {
    ...
    Ext.lt.pmkpi.pageinit = function () {
        if (_viewtype != "query") {
            //$('#maintable').append("<button title='新增指标' onclick=\"deptperftest.add();\" type=\"button\" style=\"float:left;margin:1px 10px 0 15px;background-color:#3472ef;height: 30px;line-height: 30px;\" class=\"layui-btn btn-black\">新增指标</button>");
            //$('#maintable').append("<button title='修改指标' onclick=\"deptperftest.mod();\" type=\"button\" style=\"float:left;margin:1px 10px 0 15px;background-color:#3472ef;height: 30px;line-height: 30px;\" class=\"layui-btn btn-black\">修改指标</button>");
            $('#maintable').append("<button title='删除指标' onclick=\"deptperftest.del();\" type=\"button\" style=\"float:left;margin:1px 10px 0 15px;background-color:#3472ef;height: 30px;line-height: 30px;\" class=\"layui-btn btn-black\">删除指标</button>");
            $('#maintable').append("<button id='copybtn' title='复制总体' onclick=\"deptperftest.copy();\" type=\"button\" style=\"float:left;margin:1px 10px 0 15px;background-color:#3472ef;height: 30px;line-height: 30px;\" class=\"layui-btn btn-black\">复制总体</button>");
            // $('#maintable').append("<button title='导入' onclick=\"deptperftest.import();\" type=\"button\" style=\"float:left;margin:1px 10px 0 15px;background-color:#3472ef;height: 30px;line-height: 30px;\" class=\"layui-btn btn-black\">导入</button>");
        }
    }
    /**
     * 根据页签动态配置表格的列.
            ...
            if (col["colcode"] == "obligate1") {
                if (_viewtype != "query") {
                    ...
                    col.fn = function (l, c, data, col) {
                        var html = '<a title = \"新增\" style=\"color:#468ac9;cursor:pointer;\" onclick=\'javascript:addbtnrow1("","' + data.guid + '")\'>新增</a>&nbsp;&nbsp;' +
                            '<a title = "删除" style="color:#468ac9;cursor:pointer;" onclick=\'javascript:indexdel("' + data.guid + '","' + data._locationposition + '","' + data.sindex + '")\'>删除</a>';
                        return html;
                    }
                }
            }
    ...
    cmp.addbtnrow = function (obj, selguid) {
        Ext.lt.ui.alert("自己的新增！", function () {
        });
        var data = [];
        if (!selguid) {
            data = cmp.getSeletdata("1");
        ...
        newdata.levelno = 3;
        newdata.isleaf = 1;
        newdata.superid = data[0].superid;
        newdata.sindex = data[0].sindex;
        newdata.findex = data[0].findex;
        newdata.lv1_perf_ind_name = data[0].lv1_perf_ind_name;
        newdata.lv2_perf_ind_name = data[0].lv2_perf_ind_name;
            ...
            if (ret) {
                ...
                if (totalgoals.length > 0) {
                    ...
                    for (var i = 0; i < tempgoals.length; i++) {
                        if (yeargoals[i]["rowtype"] != "add") {
                            tempgoals[i]["guid"] = yeargoals[i]["guid"];
                            yearindexs = yeargoals[i]["#indexs"];
                        } else {
                        ...
                        if (yearindexs.length > 0) {
                            for (var k = 0; k < yearindexs.length; k++) {
                                tempindexs.push(yearindexs[k]);
                            }
                        }
                        yeargoals = [];
...
function addbtnrow1(obj, selguid) {
    var data = [];
    if (!selguid) {
        data = cmp.getSeletdata("1");
        if (data.length != 1) {
