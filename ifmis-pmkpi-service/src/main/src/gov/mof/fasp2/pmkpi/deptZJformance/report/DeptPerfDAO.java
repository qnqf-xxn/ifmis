...
public class DeptPerfDAO extends PmkpiDAO {
	...
	public List<Map<String, Object>> findIndexPros(String mainguid) {
		String sql = "select t.pro_id,t.indexguid,t.pro_code from v_perf_bgt_indexpro t where t.mainguid = ?";
	...
	public int updateGroupSindx(String agencyguid,String mainguid) {
		String sql = "update V_BGT_PERF_INDICATOR c set c.lv2_perf_ind_code = (" +
				" select k.rn from (select '4'||row_number() over(order by sname desc) rn,t.sname from V_BGT_PERF_INDICATOR t" +
				" where t.mainguid= ? and t.agencyguid= ? group by t.sname,t.agencyguid) k where k.sname = c.sname)," +
				" c.sindex = (select k.rn from (select '4'||row_number() over(order by sname desc) rn,t.sname from V_BGT_PERF_INDICATOR t" +
				" where t.mainguid= ? and t.agencyguid= ? group by t.sname,t.agencyguid) k where k.sname = c.sname)," +
				" c.lv2_perf_ind_name = (select k.sname from (select '4'||row_number() over(order by sname desc) rn,t.sname from V_BGT_PERF_INDICATOR t" +
				" where t.mainguid= ? and t.agencyguid= ? group by t.sname,t.agencyguid) k where k.sname = c.sname)" +
	...
	public int updateGroupTindx(String agencyguid,String mainguid) {
		String sql = "update V_BGT_PERF_INDICATOR c set c.code = c.lv2_perf_ind_code||LPAD(c.ordernum,8,'0'),c.lv3_perf_ind_code = c.lv2_perf_ind_code||LPAD(c.ordernum,8,'0'),c.lv3_perf_ind_name = c.name where c.agencyguid = ? and  c.mainguid = ?";
