...
public class DeptAuditDAO extends PmkpiDAO {
    public static final String PERF_TABLE_DEPTEVAL = "v_perf_t_deptevaluation";

    /**
     * 部门评价规范表 PM_PERF_DEPT_EVAL
     */
    public static final String PERF_TABLE_DEPT_EVAL = "V_PM_PERF_DEPT_EVAL";
    ...
    public void updateDeptEvalTabelDatas (List<Map<String, Object>> datas, String actioncode) throws AppException {
        if(datas != null && datas.size()>0) {
            ...
            List<String> upguids = new ArrayList<>();
            ...
            String gfsql = this.createInSql("eval_id",addguids);
            if(addguids.size() > 0) {
                //查询需要同步的数据
                List<Map<String, Object>> maindata = this.getDatas(PERF_TABLE_DEPTEVAL, sql,null);
                ...
                if (filedata != null && filedata.size() > 0) {
                    for (Map filemap : filedata) {
                        ...
                        filemap.put("report_type_code", "2");//绩效评价报告类型代码 1.单位自评 2.部门评价 3.财政评价
                        filemap.put("report_type_name", "部门评价");//绩效评价报告类型名称
                ...
                if (mainlist.size() > 0) {
                    this.saveAll(mainlist, PERF_TABLE_DEPT_EVAL);
                }
                if (filelist.size() > 0) {
            ...
            if(delguids.size()>0){
                ...
                this.updateAllByColumn("eval_id", delguids, this.PERF_TABLE_DEPT_EVAL, " eval_id = sys_guid(),"+delsql, null);
     ...
     * @return
     */
    public Map<String, Object> setDataMap(Map<String, Object> map, String type){
        if (map != null){
            String agencyguid = (String) map.get("agencyguid"); //单位guid
