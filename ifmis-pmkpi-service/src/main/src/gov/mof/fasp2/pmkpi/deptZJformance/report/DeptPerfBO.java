...
public class DeptPerfBO extends PmkpiBO {
    ...
    public String getVodSql(Map sqlmap) throws AppException {
        ...
        returnsql.append(deptPerfDAO.PERF_DEPTP_ERFDECLARE).append(" t where 1=1");
    ...
    public Map<String, Object> getTreeData(String menuId) throws AppException {
        ...
        if (treesource != null && !StringUtils.isEmpty(treesource)){
            ...
            List datas = deptPerfDAO.getLeftTree(PmkpiDAO.PMKPI_TABLE_T_AGENCY, sql);
...
//        String componentid = (String) config.get("componentid"); //编辑区组件
        ...
        if (guid != null && !guid.isEmpty()) {
			...
			if (viewtype != null && "mod".equals(viewtype)) {//修改
                ...
                //处理部门整体绩效目标
                if ("v_bgt_perf_goal_info".equals(tablecode)){
                    String  kpi_depid = StringUtil.createUUID();
                    infodata.put("guid", kpi_depid);
                    infodata.put("kpi_dep_id", kpi_depid);
                    ...
                    try {
                        eledatas = dss.getRangesByWhereSql("PMKPIAGENCYALLPAY", "guid='" + saveAgency + "'");
                    } catch (AppException e) {
                        e.printStackTrace();
                    }
                    if (eledatas.size() > 0) {
                        infodata.put("dept_name", eledatas.get(0).get("name"));
                    }
                    infodata.put("mof_div_name", CommonUtil.getProvinceNameByCode((String) infodata.get("mof_div_code")));
                    infodata.put("is_last_inst",2);
                    infodata.put("is_backup",2);//是否终审
                    infodata.put("versiontype",0);
                    infodata.put("version", PerfConstant.PMKPI_VERSION_RCDTWH);//版本号
                    infodata.put("version_name", PerfConstant.PMKPI_VERSION_NAME_RCDTWH);//版本号说明
                }
...
//        String agency = (String) params.get("saveAgency");
        String tablecode = (String) params.get("tablecode");
    ...
    public Map<String, Object> del(HashMap<String, Object> parms) {
        ...
        try {
            ...
            deptPerfDAO.updateIsDeleteDatas("V_BGT_PERF_GOAL_INFO", delsql);
            delsql = " exists(select 1 from " + deptPerfDAO.PMKPI_TABLE_TMP_AGENCY + " a where a.guid=t.mainguid)";
            deptPerfDAO.updateIsDeleteDatas("V_BGT_PERF_INDICATOR", delsql);
            delsql = " exists(select 1 from " + deptPerfDAO.PMKPI_TABLE_TMP_AGENCY + " a where a.guid=t.mainguid)";
            deptPerfDAO.updateIsDeleteDatas("V_PERF_BGT_INDEXPRO", delsql);
            delsql = " exists(select 1 from " + deptPerfDAO.PMKPI_TABLE_TMP_AGENCY + " a where a.guid=t.mainguid)";
            deptPerfDAO.updateIsDeleteDatas("V_PERF_BGT_LINKBGTINFO", delsql);
            backMap.put("flg","1");
        } catch (AppException e) {
...
//        String saveAgency = (String)params.get("saveAgency");
        ...
        if ("indexprodatatable".equals(componentid)){ //处理部门预算指标对应项目关系
            tablecode ="V_PERF_INDEXTOPROS";
        }
        data = deptPerfDAO.getDatas(tablecode, wheresql, orderby);
        if ("budgetdatatable".equals(componentid)){ // 预算挂钩机制 有问题，得处理下
            List<Map<String, Object>> bgtdata =  new ArrayList<>();
            wheresql = wheresql + " and t.isbudget = 1";
            orderby = " order by t.ordernum";
            List<Map<String, Object>> bgtindicators = deptPerfDAO.getDatas("V_BGT_PERF_INDICATOR", wheresql, orderby);
            if (data.size() == 0 ){
        ...
        if ("perfindexdatatable".equals(componentid)){ //部门指标
            //部门绩效指标组件，查询选择的项目信息
            List<Map<String, Object>> projects = PerfUtil.lowMapKey(deptPerfDAO.findIndexPros(mainguid));
    ...
    public Map<String, Object> tablesave(HashMap<String, Object> params) throws AppException {
        ...
        //查询层次分类
//        List<Map<String, Object>> tempdatas = this.getTempdatas("PM001");
        for (Map<String, Object> map : datas) {
            ...
            if (rowtype != null && "add".equals(rowtype)) {
                ...
                if (tablecode.equalsIgnoreCase("V_BGT_PERF_INDICATOR")){
                    map.put("sindex","41"); //后处理
                    map.put("code","410000009"); //后处理
                    //map.put("name","自定义三级"); //后处理
                    ...
                    map.put("lv2_perf_ind_name",map.get("sname"));
                    map.put("lv3_perf_ind_code","410000009"); //后处理
                    map.put("lv3_perf_ind_name",map.get("name"));
                    if(StringUtil.isNullOrEmpty((String) map.get("kpi_content"))){
                        map.put("kpi_content",map.get("name")); //指标内容如果没有默认给指标名称
                    }
                }
                addlist.add(map);
            } else if (rowtype != null && "mod".equals(rowtype)){ //预算编制挂钩机制
                bgtmodlist.add(map);
            }
        }
        if (addlist.size() > 0) {
            //deptPerfDAO.setIndexDatas(addlist,"DEPT",false);
            ...
            if ("perfindexdatatable".equals(componentid)){ //处理指标对应的项目s
                ...
                deptPerfDAO.saveAll(allprojs, "V_PERF_BGT_INDEXPRO");
            }
        }
        /**
         * 处理修改的行
...
//                List<Map<String, Object>> modprojs = new ArrayList<>();
     ...
     * @throws AppException -- 自定义异常.
     */
    public List<Map<String, Object>> findWaitPros(String agencyguid,String wheresql) {
        return deptPerfDAO.findWaitPros(agencyguid,wheresql);
    }
    ...
    public Map<String, Object> copyPerf(HashMap<String, Object> params) throws AppException {
        ...
        List<Map<String, Object>> data = deptPerfDAO.getDatas(tablecode, querysql, orderby);
        if(copytype!=null && "index".equals(copytype)){
            data.forEach(map ->{
                ...
                map.put("kpi_id",guid);
                map.put("mainguid", mainguid);
                map.put("kpi_val3",map.get("kpi_val2"));
                map.put("kpi_val2",map.get("kpi_val1"));
                map.put("kpi_val1",map.get("kpi_val2023"));
                map.put("kpi_val2023",map.get("kpi_val"));
                map.put("rowtype","add");
                map.put("snindexval","");//202X指标值 不复制
                map.put("indexval","");//202X指标值 不复制
                map.put("kpi_val","");//202X指标值 不复制
                map.put("projects","");//部门预算项目的关系 不复制
            });
        }else if(copytype!=null && "goal".equals(copytype) && data.size()>0){//前台页面不修改编辑内容直接点保存不触发保存方法 直接更新数据
            ...
            List<Map<String, Object>> nowlist = deptPerfDAO.getDatas(tablecode, querysql, orderby);
            ...
            if(nowlist.size()>0){
                ...
                cols.remove("province");
                cols.remove("year");
                deptPerfDAO.update(cols, goalmap, tablecode);
            }else{
                goalmap.put("createtime", timeStamp);
                goalmap.put("updatetime", timeStamp);
                goalmap.put("update_time", timeStamp);
                goalmap.put("create_time", timeStamp);
                deptPerfDAO.save(goalmap, tablecode);
            }
        }
        backMap.put("data", data);
        return backMap;
