 ...
 */
public class EvaluResultBO extends PmkpiBO {
    private EvaluResultDAO evaluResultDAO;

    public void setEvaluResultDAO(EvaluResultDAO evaluResultDAO) {
        this.evaluResultDAO = evaluResultDAO;
    }
...
//        String bustype = (String)params.get("bustype");
    ...
    public Map<String, Object> save(HashMap<String, Object> params) throws AppException {
        ...
        Map map = null;
        List<Map> updatas = evaluResultDAO.getByGuid(tablecode, mainguid);
		if (updatas != null && updatas.size() > 0) {//修改
			map = updatas.get(0);
			map.putAll(datas);
			map.put("updatetime", PerfUtil.getServerTimeStamp());
			evaluResultDAO.update(map, tablecode);
		} else { //新增
            ...
            evaluResultDAO.save(datas, tablecode);
        }
